...
Note that ~data.parties.company[0].roles~ offers a list of all the roles, including synthesized roles, relevant to the current round.

Note that ~data.parties.<EVERYBODY-ELSE>[N]._role~ offers a list of all the roles vis-a-vis the company, including roles synthesized for the current round.
...
A given template (in this case the /debug/ template0 may have further attributes of its own: ~data._config.templates.dict2.debug.style~ is ~"sketchRockwell"~. That's a font, by the way.
