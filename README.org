...
The version of /Available Templates/ in your cloned Frog Porridge spreadsheet doesn't actually do anything. By default, every time the user runs /Generate PDFs/, the script goes off and reads the main Demo Master's /Available Templates/ sheet. The local copy in your spreadsheet is useful for reference, but don't bother changing anything in it, because those changes won't be picked up. It's just there for reference.

Go look at your /Available Templates/ sheet. You will see that each template has a handful of attributes:
- name
- title
- URL
- To
...
After the end-user has filled in the spreadsheet with all the details of their startup, in what form does that information become available to you, the template author?

We assume basic familiarity with Google Apps Script's HTML Template service. If you don't know what <?= ?> means, go for [[https://developers.google.com/apps-script/guides/html/templates][a quick refresher]].
