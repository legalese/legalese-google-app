...
~fillTemplates~ lives in ~templates.js~. First ~fillTemplates~ checks if it's running in the [[*The Controller][controller context]]. If not, it reads the current sheet (using ~readRows()~). It determines which templates are desired. It fills each template, and produces XML in the output folder in Google Drive.
