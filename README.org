...
Just because a row looks white-on-black doesn't mean it's special. We look not at the color of its skin, but at the content of the cell. Martin Luther King, dig?
...
*** Parties and Roles Synthesized from the Cap Table

Suppose you're working a deal that's five rounds into the history of the company.

For the sake of this example, let's say that in previous rounds,
...
- compute whether any existing securities convert
...
- compute the vesting progress of the company's ESOP plan

Legalese makes it easy for you to do all these things.

Why do these things need to happen? Let's look at a couple of case studies.
...
That template is called the ~preemptive_waiver~, because most of the existing shareholders respond with a stock answer: they waive their preemptive rights and allow the new investor to purchase the shares. If any of the existing shareholders decide to participate in the round, they don't need to send back the waiver -- the fact of their subscription for the current securities is /prima facie/ evidence that they received the preemptive notice and are responding positively.
...
**** The CapTable object
Arguably these things should be available in a ~captable~ object.
**** old_investor
**** new_investor
**** brand_new_investor
**** shareholder
...
Legalese also generates /notices/ and /resolutions/, which are not exactly contracts, but are similar to contract,s in that they require signatures.
*** singlepage.indt
