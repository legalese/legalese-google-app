...
Legalese also comes with utility templates, which get invoked as part of a bigger template. For example, we have a template that expands to a signature block. If your SAFE needs five people to sign it, the SAFE template will /include/ the signature template five times.
...
All of these entities are recorded in the /Entities/ tab. You'll notice that the /Entities/ tab has a certain structure: the first few lines in the hseet are title and prelude; then a dark gray line begins with the section header *ENTITIES*. The rest of the line has column headings which define the attrbutes for each entity.
...
Don't leave blank spaces in the section header line between attribute names. I don't know why I even have to say this.
...
Legalese sees the history of your startup as a series of deals. Yeah, we know, there's blood, sweat, and tears, and when you read /The Hard Thing About Hard Things/ you saw yourself in the starring role, and you may have lovingly chronicled the life of your startup in a thousand late-night tweets.
...
**** Skip Exploding
Sometimes someone should be left out from the exploder list. If that's true, in a *ROLES* section somewhere, set their entity attribute to ~skip_exploder: true~. Or ~skip_exploding: true~. I could never keep them straight in my head, so both forms work.
...
Now it's time to deal a deal: to generate some paperwork that materially changes the state vector of your corporation.
...
If Column A contains the word ~ignore:~ then the spreadsheet parser ignores that row.
...
If your template was defined with /Explode/ : /director/ then you can expect to see ~data.director~ available.
...
*** Roles Synthesized from the Cap Table
...
The cap table contains all the information needed to compute the ~sitout_shareholder~ role. Any entity which was previously a shareholder in the company, but is not re-upping -- not participating in the current round -- is a ~sitout_shareholder~. That's a good example of a /synthetic role/.

A *synthetic role* is a role which Legalese computed on the fly, as opposed to an /explicit role/.
...
But investor-type roles can be deduced from the cap table, and are prime candidates for /synthetic roles/.
...
Sorry for the confusing nomenclature; it evolved organically.

You do not need to explicitly define any of the above roles: they get computed by Legalese. Like any explicitly defined role, they may be used in /Available Templates/ and in your templates.

What does ~shareholder~ mean? I don't know. I don't think it's synthesized. It should be; it should describe anyone who owns shares in the company, and who appears on the register of members -- as opposed to an investor who owns debt. Debtholders don't have to attend annual general meetings and don't have to sign members' resolutions.

**** The CapTable object
Arguably these things should be accessble from a ~captable~ object.
