...
~fillTemplates~ lives in ~templates.js~. First ~fillTemplates~ checks if it's running in the [[*The Controller][controller context]]. If not, it reads the current sheet (using ~readRows()~). It determines which templates are desired. It fills each template using Google Apps's [[https://developers.google.com/apps-script/guides/html/templates][HTML Service]], and saves the resulting valid XML in the output folder in Google Drive. It shares the output folder with robot@legalese.io.
...
~inc_signature~ is called by ~inc_enumerate_signatures~.
